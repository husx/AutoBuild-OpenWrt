...
cat >$NETIP <<-EOF
uci set network.lan.ipaddr='192.168.10.201'                                    # IPv4 地址(openwrt后台地址)
...
sed -i "s/OpenWrt /${Author} compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt Lede_x86-64，Luci 18.06 /g" $ZZZ          # 增加个性名字${Author}默认为你的github账号
